		...
		const accountHtml = `
            <div class="table__row">
                ...
                <div class="table__cell table__cell_content_deposits" data-label="Deposits">
                    <p class="table__cell-text">${Number(
			account.sumDeposits
		).toFixed(2)}</p>
                    <button class="table__detail-btn" onclick="openDetailInfoDeposits(${account.id
			})"></button>
                </div>
                <div class="table__cell table__cell_content_withdraws" data-label="Withdraws">
                    <p class="table__cell-text">${Number(
				account.sumWithdraws
			).toFixed(2)}</p>
                    <button class="table__detail-btn" onclick="openDetailInfoWithdraws(${account.id
			})"></button>
                </div>
                <div class="table__cell table__cell_content_balances" data-label="Balances">
                    <p class="table__cell-text">${account.lastBalance || "?"
			}</p>
                    <button class="table__detail-btn" onclick="openDetailInfoBalance(${account.id
			})"></button>
                </div>
                <div class="table__cell table__cell_content_bets" data-label="Bets">
                    <p class="table__cell-text">${account.countBets}</p>
                    <button class="table__detail-btn" onclick="openDetailInfoBetsAccount(${account.id
			})"></button>
                </div>
                <div class="table__cell table__cell_content_session" data-label="Session">
                    <p class="table__cell-text">${account.countSessions}</p>
                    <button class="table__detail-btn" onclick="openDetailInfoSession(${account.id
			})"></button>
                </div>
                <div class="table__cell table__cell_content_last-use" data-label="Last use">
                    <p class="table__cell-text">${account.lastUseDateConverted
			}</p>
                </div>
                <div class="table__cell table__cell_content_creation-date" data-label="Creation date">
                    <p class="table__cell-text">${account.createDateConverted
			}</p>
                </div>
                <div class="table__cell table__cell_content_status" data-label="Status">
                    <div class="table__select select">
                        <div class="select__input" data-value="${account.status || "Выберите"
			}" onclick="openSelector(this)">${account.status || "Выберите"
			}</div>
                    ...
                    <button class="table__detail-btn" onclick="openDetailInfoStatus(${account.id
			})"></button>
                </div>
            </div>`;
...
async function searchTableAccounts(event) {
	...
	let input = document
		.getElementById("searchInputAccounts")
		.value.toLowerCase();
	if (input === "") {
			...
			try {
                ...
                <div class="popup__transaction-block">
                    <div class="popup__transaction-size">
                        <p class="popup__transaction-currency">${obj.currency
					}</p>
                        <p class="popup__transaction-amount">${obj.amount || "N/A"
					}</p>
                    </div>
                    <p class="popup__transaction-date">${formatDate(
						obj.timestamp
					)}</p>
                </div>
                `;
			...
			try {
				...
				const accountHtml = `
					...
					obj.timestamp
                                ...
                                <p class="popup__bet-status">${obj.status || "N/A"
					}</p>
                            </div>
                            <div class="popup__bet-size">
                                ...
                                <p class="popup__payout-currency">${obj.currency || "N/A"
					}</p>
                                <p class="popup__payout-amount">${obj.payout || "N/A"
					}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="popup__session-block">
                    <div class="popup__session-info">
                        <div class="popup__session-token-block">
                            <p class="popup__session-token" onclick="redirectionToToken(this)">${obj.token
					}</p>
                            <p class="popup__session-description">(${obj.description || ""
					})</p>
                        </div>
                        <p class="popup__session-hash" onclick="redirectionToHash(this)">${obj.hash
					}</p>
                    </div>
                    <p class="popup__session-date">${formatDate(
						obj.timestamp
					)}</p>
                </div>
                `;
				...
				const accountHtml = `
                    <div class="status-change-popup__status-change-block">
                        <div class="status-change-popup__status-info">
                            <p class="status-change-popup__status-value">${obj.oldStatus
					}</p>
                            <p class="status-change-popup__status-separator">➔</p>
                            <p class="status-change-popup__status-value">${obj.newStatus
					}</p>
                        </div>
                        <div class="status-change-popup__additional-info">
                            <p class="status-change-popup__changer">${obj.editor
					}</p>
                            <p class="status-change-popup__date">${formatDate(
						obj.timestamp
					)}</p>
                        </div>
                    </div>
