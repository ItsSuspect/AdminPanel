...
function openEditUser(element) {
	...
	document
		...
		.setAttribute("data-userId", element.getAttribute("data-userId"));

	document.getElementById("application-name-user-edit").value =
		element.getAttribute("data-app");
	document.getElementById("token-user-edit").value =
		element.getAttribute("data-token");
	document.getElementById("description-user-edit").value =
		element.getAttribute("data-description");

	const timestampInMilliseconds =
		element.getAttribute("data-expirationDate") * 1000;
...
function openBanUser(userId) {
	...
	document
		.getElementById("ban-user-button")
...
async function editUser(element) {
	const dateInput = document.getElementById(
		"expiration-date-user-edit"
	).value;
	const timeInput = document.getElementById(
		"expiration-time-user-edit"
	).value;
...
async function banUser(element) {
	try {
		let response = await fetch(
			"/pg_ru/banUser/" + element.getAttribute("data-userId"),
			{
				method: "POST",
				priority: "high",
			}
...
async function deleteUser(element) {
	try {
		let response = await fetch(
			"/pg_ru/deleteUser/" + element.getAttribute("data-userId"),
			{
				method: "POST",
				priority: "high",
			}
