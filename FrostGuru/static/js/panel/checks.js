...
document.addEventListener("DOMContentLoaded", async () => {
	...
	if (checks.length < batchSizeChecks) button.style.display = "none";
...
function getTableRowContentCheck(check) {
	...
	return `
        ...
        <div class="table__cell table__cell_content_domain" data-label="Domain">
            <p class="table__cell-text table__cell-text_clickable" onclick="copyDomain(this)">${check.domain}</p>
        </div>
...
function copyDomain(element) {
	const domain = element.textContent

	navigator.clipboard
		.writeText(domain)
		.then(async () => {
			await sendNotification("Копирование", "Копирование успешно!", "success");
		})
		.catch(async () => {
			const tempTextArea = document.createElement("textarea");
			tempTextArea.value = domain;
			...
			try {
				...
				await sendNotification("Копирование", "Копирование не успешно!", "error");
			}
	...
	try {
		...
		if (response.ok) {
			...
			if (data.success) {
				...
				document.querySelector('.table__content .table__row[data-checkId="' + element.getAttribute("data-checkId") + '"]').remove();
...
async function exportToExcel() {
	try {
		const startDate = new Date(document.getElementById("start-date-excel").value);
		const startTimestamp = startDate.setHours(0, 0, 0, 0) / 1000;

		const endDate = new Date(document.getElementById("end-date-excel").value);
		const endTimestamp = endDate.setHours(23, 59, 59, 999) / 1000;

		const body = {
			startDate: startTimestamp,
			endDate: endTimestamp,
			language: document.getElementsByClassName("detailing-popup__language-btn")[0].innerText
		};
