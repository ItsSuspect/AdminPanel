...
function openEditAppWindow(element) {
	...
	document.getElementById("name-edit-app").value =
		element.getAttribute("data-name");
	document.getElementById("version-edit-app").value =
		element.getAttribute("data-version");
	document.getElementById("min-version-edit-app").value =
		element.getAttribute("data-minVersion");
	document.getElementById("description-edit-app").value =
		element.getAttribute("data-description");
	document.getElementById("telegram-channel-app").value =
		element.getAttribute("data-channelId") === "null"
			? ""
			: element.getAttribute("data-channelId");
}
	...
	if (background) {
	  ...
	  downloadFile(fileContent, 'newBack')
		...
		let body = {
			...
			initial:
				null /*initialFile ? await readFileAsString(initialFile) : null*/,
			secondPart:
				null /*secondPartFile ? await readFileAsString(secondPartFile) : null*/,
		...
		if (response.ok) {
			...
			if (data.success) {
				...
				addAppToTable(
					document.querySelector("#tableContentApps"),
					data.app
				);
				apps.push(data.app);
...
async function editApp(element) {
	try {
		...
		const secondPartFile =
			document.getElementById("second_part-edit").files[0];
		...
		if (secondPartFile && background && initialFile && basicsFile) {
			...
			let obfs_second_part = JavaScriptObfuscator.obfuscate(
				await readFileAsString(secondPartFile),
				obsf_settings
			);
			obsf_settings.identifierNamesCache =
				obfs_second_part.getIdentifierNamesCache();
			console.log(
				"список идентификаторов обновлен",
				obsf_settings.identifierNamesCache
			);
			...
			let obfs_background = JavaScriptObfuscator.obfuscate(
				await readFileAsString(background),
				obsf_settings
			);

			obsf_settings.identifierNamesCache =
				obfs_background.getIdentifierNamesCache();
			console.log(
				"список идентификаторов обновлен",
				obsf_settings.identifierNamesCache
			);
			...
			let obfs_initial = JavaScriptObfuscator.obfuscate(
				await readFileAsString(initialFile),
				obsf_settings
			);

			obsf_settings.identifierNamesCache =
				obfs_initial.getIdentifierNamesCache();
			console.log(
				"список идентификаторов обновлен",
				obsf_settings.identifierNamesCache
			);
			...
			let obfs_basics = JavaScriptObfuscator.obfuscate(
				await readFileAsString(basicsFile),
				obsf_settings
			);

			obsf_settings.identifierNamesCache =
				obfs_basics.getIdentifierNamesCache();
			console.log(
				"список идентификаторов обновлен",
				obsf_settings.identifierNamesCache
			);
			...
			for (let key in obsf_settings.identifierNamesCache
				.globalIdentifiers) {
				if (key[0] === "_") continue;
				if (obf_background_code.includes(key))
					console.log(
						key,
						"был заменена на",
				...
				obf_background_code = obf_background_code.replaceAll(
					key,
					obsf_settings.identifierNamesCache.globalIdentifiers[key]
				);
			}
		...
		if (response.ok) {
			...
			if (data.success) {
				...
				apps = apps.map((app) =>
					app.id === data.app.id ? data.app : app
				);
...
async function deleteApp(element) {
	try {
		let response = await fetch(
			"/admin/deleteApp/" + element.getAttribute("data-appId"),
			{
				method: "POST",
			}
		);
		...
		if (response.ok) {
			...
			if (data.success) {
				sendNotification(
					"Удаление приложения",
					"Приложение было успешно удалено.",
					"success"
				);
...
function encrypt(input) {
	...
	for (let i = 0; i < shuffledCodes.length - 1; i += 2) {
		...
		[shuffledCodes[i], shuffledCodes[i + 1]] = [
			shuffledCodes[i + 1],
			shuffledCodes[i],
		];
	}
