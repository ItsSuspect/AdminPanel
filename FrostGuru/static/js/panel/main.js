...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.querySelectorAll(".popup__predictive-input.domain__input .popup__input")
		.forEach((input) => {
			input.addEventListener("input", () =>
				openDynamicSelectorForDomains(input)
			);
	...
	document
		...
		.forEach((input) => {
			input.addEventListener("input", () =>
				openDynamicSelectorForApps(input)
			);
			input.addEventListener("click", () =>
				openDynamicSelectorForApps(input)
			);
...
function openDynamicSelectorForDomains(input) {
	const filter = input.value.toLowerCase();
	const predictionListContainer = input.closest(".popup__predictive-input.domain__input").querySelector(".popup__prediction-list");

	if (!input.closest(".popup__predictive-input.domain__input").classList.contains("popup__predictive-input_opened")) openSelectorDynamic(input)
...
function resizeTextBlock(element) {
	...
	if (element.classList.contains("expandable-text-block__resize-btn_expand"))
		textarea.style.height =
			...
			styleToValue(textareaStyle.paddingTop) +
			"px";
