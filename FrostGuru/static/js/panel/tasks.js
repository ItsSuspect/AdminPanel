document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".tasks-container").forEach((taskContainer) => {
    taskContainer.addEventListener("click", (event) => {
      ...
      if (taskText.hasAttribute("style")) taskText.removeAttribute("style");
...
async function loadMoreTasks(executor) {
  ...
  try {
    ...
    if (data.length > 0) {
      ...
      if (data.length < 10)
        document.getElementById(`LoadMoreButton-${executor}`).style.display =
          "none";
    } else {
      document.getElementById(`LoadMoreButton-${executor}`).style.display =
        "none";
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ...
        document
          .querySelector('.task-element[data-taskId="' + taskId + '"]')
          .remove();
        tasks = tasks.filter((task) => task.id !== taskId);

        const container = document.getElementById(
          `executor-container-${element.getAttribute("data-taskExecutor")}`
        );
