    ...
    const logHtml = `
        <div class="table__row">
            ...
            <div class="table__action-block">
                <button class="table__action-btn table__action-btn_action_detail-info" onclick="openDetailInfoLog(${log.id})"></button>
            </div>
        </div>
    `;
...
async function searchTableLog(event) {
    ...
    let input = document.getElementById('searchInputActivity').value.toLowerCase();
    ...
    if (input === '') {
        ...
        if (logs.length > batchSize) loadMoreButton.style.display = 'block';
    ...
    try {
        ...
        if (data.length > 0) {
            ...
            tableContentLogs.innerHTML = '<div class="table__no-data">Данные отсутствуют</div>';
        }
...
function openDetailInfoLog(logId) {
    ...
    document.getElementById('action-detail-popup-activity').style.display = 'block';
